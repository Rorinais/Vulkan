...
//	Cube cube;
//	mMesh= Mesh(cube.generateGeometry(),mLogicalDevice,mCommandPool);
//
//	mMaterialWarehouse = MaterialWarehouse(mLogicalDevice);
//
//
//    createPipeline();
...
//    mPipeline = Pipeline::create(mLogicalDevice, mMaterialWarehouse.getMaterial(mMesh.getMaterial())->getPipelineState());
//
...
//    //rasterization.setPolygonMode(VK_POLYGON_MODE_LINE) 
//    //    .setLineWidth(1.0f)                  
...
//    mPipeline->setShaderStage(mMaterialWarehouse.getMaterial(mMesh.getMaterial())->getShaderProgram());
//
...
//        mCommandBuffers[i]->bindVertexBuffers(mMesh.getVertexBuffer()->getBufferHandles());
//        mCommandBuffers[i]->bindIndexBuffer(mMesh.getIndexBuffer()->getBuffer());
//        //mCommandBuffers[i]->drawIndexed(static_cast<uint32_t>(indices.size()));
//        mCommandBuffers[i]->drawIndexed(static_cast<uint32_t>(mMesh.getGeometry()->getIndices().size()));
