...
Texture::Texture(VulkanContext& context, const char* imagePath) : mContext(context) {
...
// 深度纹理专用构造函数
Texture::Texture(VulkanContext& context, Type type, VkExtent2D extent)
    : mContext(context), mType(type)
{
    if (type != Type::Depth)
        throw std::runtime_error("Invalid constructor for non-depth texture");

    mFormat = findSupportedDepthFormat(context.physicalDevice);
...
Texture::~Texture() {
    if (mImageView != VK_NULL_HANDLE) {
        vkDestroyImageView(mContext.logicalDevice, mImageView, nullptr);
        mImageView = VK_NULL_HANDLE;
    }
    if (mSampler != VK_NULL_HANDLE) {
        vkDestroySampler(mContext.logicalDevice, mSampler, nullptr);
        mSampler = VK_NULL_HANDLE;
    }
    if (mImage != VK_NULL_HANDLE) {
        vkDestroyImage(mContext.logicalDevice, mImage, nullptr);
        mImage = VK_NULL_HANDLE;
    }
    if (mMemory != VK_NULL_HANDLE) {
        vkFreeMemory(mContext.logicalDevice, mMemory, nullptr);
...
void Texture::createImage(VkFormat format, VkExtent2D extent, VkImageUsageFlags usage, VkImageTiling tiling)
{
    ...
    if (vkCreateImage(mContext.logicalDevice, &imageInfo, nullptr, &mImage) != VK_SUCCESS) {
...
void Texture::allocateMemory(VkMemoryPropertyFlags properties) {
    ...
    vkGetImageMemoryRequirements(mContext.logicalDevice, mImage, &memRequirements);
    ...
    if (vkAllocateMemory(mContext.logicalDevice, &allocInfo, nullptr, &mMemory) != VK_SUCCESS) {
        throw std::runtime_error("Failed to allocate image memory!");
    }

    vkBindImageMemory(mContext.logicalDevice, mImage, mMemory, 0);
}
...
void Texture::createImageView()
{
    ...
    if (vkCreateImageView(mContext.logicalDevice, &viewInfo, nullptr, &mImageView) != VK_SUCCESS) {
        throw std::runtime_error("Failed to create texture image view!");
    }
}

void Texture::createSampler(const VkSamplerCreateInfo& samplerInfo) {
    if (vkCreateSampler(mContext.logicalDevice, &samplerInfo, nullptr, &mSampler) != VK_SUCCESS) {
...
void Texture::uploadData(const void* data, size_t dataSize, VkExtent2D extent) {
    ...
    if (vkCreateBuffer(mContext.logicalDevice, &bufferInfo, nullptr, &stagingBuffer) != VK_SUCCESS) {
        throw std::runtime_error("Failed to create staging buffer!");
    }

    VkMemoryRequirements memRequirements;
    vkGetBufferMemoryRequirements(mContext.logicalDevice, stagingBuffer, &memRequirements);
    ...
    if (vkAllocateMemory(mContext.logicalDevice, &allocInfo, nullptr, &stagingMemory) != VK_SUCCESS) {
        vkDestroyBuffer(mContext.logicalDevice, stagingBuffer, nullptr);
        throw std::runtime_error("Failed to allocate staging memory!");
    }

    vkBindBufferMemory(mContext.logicalDevice, stagingBuffer, stagingMemory, 0);

    // 拷贝数据到暂存缓冲区
    void* mapped;
    vkMapMemory(mContext.logicalDevice, stagingMemory, 0, dataSize, 0, &mapped);
    memcpy(mapped, data, dataSize);
    vkUnmapMemory(mContext.logicalDevice, stagingMemory);
    ...
    // 清理暂存资源
    vkDestroyBuffer(mContext.logicalDevice, stagingBuffer, nullptr);
    vkFreeMemory(mContext.logicalDevice, stagingMemory, nullptr);
}
...
VkCommandBuffer Texture::beginSingleTimeCommands() {
    ...
    vkAllocateCommandBuffers(mContext.logicalDevice, &allocInfo, &cmdBuf);
...
void Texture::endSingleTimeCommands(VkCommandBuffer cmdBuf) {
    ...
    vkFreeCommandBuffers(mContext.logicalDevice, mContext.commandPool, 1, &cmdBuf);
}
